...
Beyond the build and tests, three gates run on every pull request. They are listed here because meeting one unprepared looks like an unexplained failure:
...
- **Script suites** — if you touch `scripts/` or `fastlane/`, `.github/workflows/tooling.yml` runs `./scripts/run-script-tests.sh` and verifies the fastlane configuration loads. Run that script locally first; it is the same one CI calls.
