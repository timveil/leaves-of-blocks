...
#if DEBUG
struct GameTables_Previews: PreviewProvider {
    static var previews: some View {
        ScrollView {
            VStack(spacing: 30) {
                // Scoring table
                ScoringTable(scoringRules: [
                    ScoringTable.ScoringRule(
                        action: "Place Block",
                        points: "10 per cell",
                        description: "Basic placement"
                    ),
                    ScoringTable.ScoringRule(
                        action: "Clear Line",
                        points: "100",
                        description: "Complete row/column"
                    ),
                    ScoringTable.ScoringRule(
                        action: "Combo Bonus",
                        points: "50 extra",
                ...
                // Difficulty table
                DifficultyTable(difficultyLevels: [
                    DifficultyTable.DifficultyLevel(
                        difficulty: .easy,
                        description: "Smaller blocks",
                        blockFrequency: "Forgiving"
                    ),
                    DifficultyTable.DifficultyLevel(
                        difficulty: .moderate,
                        description: "Balanced mix",
                        blockFrequency: "Standard"
                    ),
                    DifficultyTable.DifficultyLevel(
                        difficulty: .hard,
                        description: "Complex shapes",
                        blockFrequency: "Challenging"
                    )
                ])
                
                // Summary stats
                SummaryStatsTable(currentStats: SummaryStatsTable.GameStats(
                    score: 1245,
                    blocksPlaced: 45,
                    linesCleared: 12,
                    gameTime: 180,
